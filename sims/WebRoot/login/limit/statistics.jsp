...
<body>
	...
	<div class="container">
		...
		<c:choose>
			<c:when test="${not empty scoreLevelList}">
				<div class="container">
					<div class="row">
						<div class="table-responsive">
							<table class="table table-striped table-bordered">
								...
								<tbody>
									<c:forEach items="${scoreLevelList}" var="obj">
										<tr>
											<td>${obj.stuId}</td>
	...
	<script>
		function lastPageAction() {
			var page = "<%=path%>/statistics?semester=${semester}&classId=${classId}&indexPage=${pageInfo.indexPage - 1}";
		...
		$(function() {
			$("#search").click(function() {
				var page = "<%=path%>/statistics?semester=";
				page += $("#semester").val();
				page += "&classId=" + $("#classId").val();
				$(location).attr('href', page);
			});
		});
	</script>
